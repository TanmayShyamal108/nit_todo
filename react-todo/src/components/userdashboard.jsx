...
export function Userdashboard() {
    ...
    useEffect(() => {
        axios.get(`http://127.0.0.1:5050/appointments/${cookies['name']}`)
            .then(response => {
    ...
    function handleCloseClick(e) {
        axios.delete(`http://127.0.0.1:5050/delete-task/${e.target.name}`)
            .then(() => {
    ...
    function handleEditClick(id) {
        alert(id);
        axios.get(`http://127.0.0.1:5050/get-task/${id}`)
            .then(response => {
    ...
    const editFormik = useFormik({
        ...
        onSubmit: (task) => {
            axios.put(`http://127.0.0.1:5050/edit-task/${task.Appointment_Id}`, task);
    ...
    return (
        <div className=" p-4" style={{ height: '100vh', width: '100%' }}>
            <h2>Welcome {cookies['name']} <button onClick={handleSignout} className='btn btn-warning'>Signout</button> </h2>
            <div className="mb-3">
